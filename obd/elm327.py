...
class ELM327:
    ...
    def __init__(self, uart, protocol, timeout,
                 check_voltage=True, start_low_power=False):
        """Initializes port by resetting device and gettings supported PIDs. """

        print("Initializing ELM327: PROTOCOL=%s" %
                    (
    ...
    # m.g. commented out because I don't use auto baud
    ...
    def __write(self, cmd):
        ...
        if self.__port:
            ...
            try:
                self.__port.read()  # dump everything in the input buffer
    ...
    def __read(self):
        ...
        while True:
            ...
            try:
                data = self.__port.read(self.__port.any() or 1)
            except Exception:
                self.__status = OBDStatus.NOT_CONNECTED
                self.__port.deinit()
                self.__port = None
