...
class Async(OBD):
    ...
    def start(self):
        ...
        if not self.is_connected():
            print("Async thread not started because no connection was made")
            return

        if len(self.__commands) == 0:
            print("Async thread not started because no commands were registered")
            return
    ...
    def watch(self, c, callback=None, force=False):
        ...
        if self.__running:
            ...
            if c not in self.__commands:
                print("Watching command: %s" % str(c))
            ...
            if hasattr(callback, "__call__") and (callback not in self.__callbacks[c]):
                print("subscribing callback for command: %s" % str(c))
    ...
    def unwatch(self, c, callback=None):
        ...
        if self.__running:
            ...
            print("Unwatching command: %s" % str(c))
    ...
    def unwatch_all(self):
        ...
        # the dict shouldn't be changed while the daemon thread is iterating
        if self.__running:
            print("Can't unwatch_all() while running, please use stop()")
        else:
        ...
        while self.__running:
            ...
            if len(self.__commands) > 0:
                ...
                for c in self.__commands:
                    if not self.is_connected():
                        print("Async thread terminated because device disconnected")
                        self.__running = False
                        self.__thread = None
                        return
