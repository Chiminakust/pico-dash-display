...
class OBD(object):
    ...
    def __init__(self, uart, protocol=None, fast=True,
        ...
        print("======================= python-OBD (v%s) =======================" % __version__)
        print('connecting ...')
        self.__connect(uart, protocol,
    ...
    def __connect(self, uart, protocol, check_voltage,
                  start_low_power):
        """
            Attempts to instantiate an ELM327 connection object.
        """

        print("Explicit port defined")
        self.interface = ELM327(uart, protocol,
    ...
    def __load_commands(self):
        ...
        if self.status() != OBDStatus.CAR_CONNECTED:
            print("Cannot load commands: No connection to car")
            return

        print("querying for supported commands")
        ...
        for get in pid_getters:
            ...
            if response.is_null():
                print("No valid data for PID listing command: %s" % get)
                continue
        ...
        print("finished querying with %d commands supported" % len(self.supported_commands))
    ...
    def __set_header(self, header):
        ...
        if not r:
            print("Set Header ('AT SH %s') did not return data", header)
            return OBDResponse()
        if "\n".join([m.raw() for m in r]) != "OK":
            print("Set Header ('AT SH %s') did not return 'OK'", header)
            return OBDResponse()
    ...
    def query(self, cmd, force=False):
        ...
        if self.status() == OBDStatus.NOT_CONNECTED:
            print("Query failed, no connection available")
            return OBDResponse()
